...
#define ETH_SW_PATCH_VERSION_CFG             2
...
[!IF "((IMPLEMENTATION_CONFIG_VARIANT != 'VariantPostBuild') and (variant:size()>1)) or ((IMPLEMENTATION_CONFIG_VARIANT = 'VariantPostBuild') and (variant:size()>0))"!][!//
    ...
    [!ENDLOOP!][!//
[!ELSEIF "((IMPLEMENTATION_CONFIG_VARIANT = 'VariantPostBuild') and (variant:size()<1))"!][!//
     [!SELECT "EthConfigSet"!][!// Select configuration
        [!VAR "ConfigSet" = "as:name(.)"!]
    [!CODE!][!WS4!]extern CONST(Eth_ConfigType, ETH_CONST) [!"$ConfigSet"!];[!CR!][!ENDCODE!]
    [!ENDSELECT!]
[!ENDIF!][!//
[!ENDNOCODE!][!//
/**
