...
#define ADC_NBR_OF_ADCHWUNIT_ADCGROUP1_CHANNELS 	ADC_CFGSET_GROUP_0_CHANNELS
...
{
    (void)Adc_ReadGroup(AdcGroup_0, ADC_GROUP_1_adcVal);
}
...
void IoHwAb_Init( void )
{
    ...
    (void)Adc_SetupResultBuffer(AdcGroup_0, adcBuf_Group1);
...
void IoHwAb_MainFunction( void )
{
    ...
    if( Initilized == TRUE ){
        Adc_StatusType state = Adc_GetGroupStatus(AdcGroup_0);
...
Adc_ValueGroupType IoHwAb_Adc_ReadSignal( Adc_GroupType group, Adc_ChannelType channel, IoHwAb_StatusType * status ) {
    ...
    if ((Adc_ChannelType)channel == (Adc_ChannelType)AdcPotChannel ) {
    	SuspendOSInterrupts();
